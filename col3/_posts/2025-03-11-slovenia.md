...
# Production (2022)  
  ...
  {% for row in site.data.slovenia_production %}
    {% if forloop.first %}
    <tr>
      {% for pair in row %}
        <th>{{ pair[0] }}</th>
...
![time series of individuals slaughtered over time](../assets/images/slovenia_timeseries.png)
...
Note: The following trade data is given as it is reported. Trade data can be inaccurate when a country imports fish for further processing and re-export. This is especially true for countries that act as a first port of entry to the EU, such as Sweden (the "Rotterdam effect").## Exports  
  ...
  {% for row in site.data.slovenia_export %}
    {% if forloop.first %}
    <tr>
      {% for pair in row %}
        <th>{{ pair[0] }}</th>
...
## Imports  
  ...
  <table>
  {% for row in site.data.slovenia_import %}
    {% if forloop.first %}
    <tr>
      {% for pair in row %}
        <th>{{ pair[0] }}</th>
